...
class SelfAttLayer_Enc(nn.Module):
    def __init__(self, time_steps=121, feature_dim=256, head_num=4, k=4, across_time=True):
        ...
        self.layer_att_ = nn.MultiheadAttention(
            embed_dim=feature_dim, num_heads=head_num, add_zero_attn=True
        )
    ...
    def forward(self, x, batch_mask, padding_mask=None, hidden_mask=None):
        ...
        if self.across_time:
            ...
            att_output, att_weight = self.layer_att_(
                q_, k, v, key_padding_mask=key_padding_mask, attn_mask=attn_mask
            )
            # att_output : [A,T,D]
...
class SelfAttLayer_Dec(nn.Module):
    ...
    def forward(self, x, batch_mask, padding_mask=None, hidden_mask=None):
        ...
        if self.across_time:
            ...
            q = (
                x_.permute(0, 2, 1, 3).reshape((-1, A, D)).permute(1, 0, 2)
            )  # [L,N,E] : [F,A,T,D]->[F,T,A,D]->[F*T,A,D]->[A,T*F,D]
