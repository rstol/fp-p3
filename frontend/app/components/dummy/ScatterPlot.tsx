...
import { getChildOrAppend, getMargin, type ChartStyle } from '~/components/utils';
 ...
 */
function renderScatterPlot(root: SVGElement | SVGGElement, data: DataArray, props: ChartStyle) {
  const margin = getMargin(props.margin);
  const height = props.height - margin.top - margin.bottom;
  const width = props.width - margin.left - margin.right;

  const visRoot = d3.select(root);
  const base = getChildOrAppend<SVGGElement, SVGElement>(visRoot, 'g', 'base').attr(
    'transform',
